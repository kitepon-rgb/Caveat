...
**v0.1 NPM 配布対応完了**。Phase 11 の v0 feature-complete に加えて Phase 12 で NPM 配布形態に再構成（2026-04-19、141 tests passing）。CLI を `caveat-cli` という単一 npm パッケージに bundle 化し、MCP サーバと Claude Code hooks を CLI のサブコマンドに統合。`caveat init` が `~/.caveat/` の scaffold と Claude Code への MCP/hook 登録まで 1 コマンドで完了させる。
...
npm install -g ./caveat-cli-0.1.0.tgz              # 別シェル/別ホストでも同様
