...
- [x] **visibility 分類の一元化**（C9）: `'public' | 'private' | 'invalid'` の分類関数を core に 1 箇所だけ置き、**厳密一致・trim/小文字化等の正規化を一切しない**（親切正規化は fail-closed 判定を緩める）。publish の収集と sealed mirror の再検証がこれを共有する。pre-commit gate は独立して `private` の完全一致をブロックし、sync は tree / visibility 非依存で remote の匿名可読性だけを判定する
