...
- [x] `gitRuntime.ts::createGit(baseDir, {timeoutMs})`: `simpleGit` に stdout/stderr の `timeout.block` 注入（background 30s / foreground 300s、命名定数+根拠コメント、5秒の先行幅を成立させるためcustom値は6000ms以上）+ `env` に `...process.env, GIT_TERMINAL_PROMPT: '0', GCM_INTERACTIVE: 'Never'`。Windowsでdirect child終了後に無応答HTTP転送中の`git-remote-http`が残る経路には、`http.lowSpeedLimit/Time`と、それを上書きする`GIT_HTTP_LOW_SPEED_LIMIT/TIME`をCaveat値で固定し、helper側をsimple-gitより5秒早い補助境界にしてGit自身の通常失敗経路で親子を畳む。これはslow-drip、DNS/TLS/SSH、一般のprocess tree、総経過時間を保証しない。PID再利用競合で無関係なprocessをkillし得るTrace2→`taskkill`方式は採用しない。**GIT_SSH_COMMAND は注入しない**〔refuter E-1: core.sshCommand / 独自 ssh config を壊す〕。community / sync / publish の全 simpleGit 呼び出しを createGit へ置換（前景にも適用 — 手動コマンドも同じ無通信ハングの穴を持つため）〔完了 a3c22a2 + 079c4ed。実装時発見: simple-git 3.36 は `.env()` 明示時に環境を vulnerabilityCheck にかけ PAGER/EDITOR 等の存在で throw するため、`@simple-git/argv-parser` の parseEnv から allowUnsafe 許可を導出（チェックと同一コードパス＝ドリフトなし）。GIT_SSH_COMMAND は非注入のまま、ユーザー環境に既在なら許可のみ〕
