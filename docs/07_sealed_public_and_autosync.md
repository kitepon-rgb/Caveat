...
- [ ] `gitRuntime.ts::createGit(baseDir, {timeoutMs})`: `simpleGit` に `timeout.block` 注入（background 30s / foreground 300s、命名定数+根拠コメント）+ `env` に `...process.env, GIT_TERMINAL_PROMPT: '0', GCM_INTERACTIVE: 'Never'`。**GIT_SSH_COMMAND は注入しない**〔refuter E-1: core.sshCommand / 独自 ssh config を壊す。hang は timeout + TERMINAL_PROMPT=0 + detached stdio で防げる〕。community / sync / publish の全 simpleGit 呼び出しを createGit へ置換（前景にも適用 — 手動コマンドも同じ無期限ハングの穴を持つため）
